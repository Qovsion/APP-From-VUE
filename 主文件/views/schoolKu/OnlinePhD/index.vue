<template>
  <div class="content">
   ...
   <img v-if="this.$store.state.OnlinePhD_lx === '5' || this.$store.state.OnlinePhD_lx === 5" src="../../../assets/images/zzbs.png"  @click="pathAcel(23)" class="datu" />
    <img v-if="this.$store.state.OnlinePhD_lx === '6' || this.$store.state.OnlinePhD_lx === 6" src="../../../assets/images/gjbs.png"  @click="pathAcel(41)" class="datu" />
...
export default {
  ...
  activated () {
    ...
    this.major()
    this.$store.state.OnlinePhD_toch = false
    this.$store.state.OnlinePhD_major = false
    this.$store.state.OnlinePhD_rules = false
  },
  created () {
    if (this.$route.query.lx === 1) {
      this.$store.state.majorMasterId = 392
  ...
  methods: {
    rmxx () {
      ...
      if (this.$route.query.lx === '5' || this.$route.query.lx === 5) {
        this.$router.push({ path: '/xxsx', query: { lx: this.$route.query.lx, name: '在职博士' } })
      }
      if (this.$route.query.lx === '6' || this.$route.query.lx === 6) {
        this.$router.push({ path: '/xxsx', query: { lx: this.$route.query.lx, name: '国际博士' } })
      }
    },
    rmzy () {
      if (this.$route.query.lx === '5' || this.$route.query.lx === 5) {
