<template>
  <div class="content">
    <van-nav-bar title="在职硕士" @click-left="$router.back()" left-arrow>
      <i slot="left" class="iconfont iconfanhui"></i>
    </van-nav-bar>
    <div class="content-con" >
      <div class="con_list" v-for="(item,i) in Zzss" :key="item.tjxh" @click="turn(item.tjxh,i)">
        <div class="no_fl">
...
export default {
  ...
  methods: {
    turn (id, i) {
      this.$router.push({ path: `/majorMaster/${id}`, query: { lx: this.Zzss[i].lx, name: this.Zzss[i].lxmc } })
    },
    onChange (event) {
      this.setData({
        value: event.detail
...
.no_fl {
  ...
  .van-tag {
    ...
    box-shadow: 0px 0px 2px 0px rgba(255,131,79,0.29) !important;
