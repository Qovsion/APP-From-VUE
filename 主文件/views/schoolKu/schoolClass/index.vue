<template>
  ...
  <div class="content">
    ...
    <div class="title">
      ...
      <div class="min_title">
        ...
        <van-tag
          color="#7CC982"
          text-color="#fff"
          v-if="schoolInfo.lxname=='同等学力'"
        >{{schoolInfo.lxname}}</van-tag>
        <van-tag
          color="#70A5FF"
          text-color="#fff"
          v-if="schoolInfo.lxname=='专业硕士'"
        >{{schoolInfo.lxname}}</van-tag>
        <van-tag
          color="#FF7977"
          text-color="#fff"
          v-if="schoolInfo.lxname=='国际硕士'"
        >{{schoolInfo.lxname}}</van-tag>
        <van-tag
          color="#FF9C4D"
          text-color="#fff"
          v-if="schoolInfo.lxname=='中外合办'"
        >{{schoolInfo.lxname}}</van-tag>
        <van-tag
          color="#878DC1"
          text-color="#fff"
          v-if="schoolInfo.lxname=='在职博士'"
        >{{schoolInfo.lxname}}</van-tag>
        <van-tag
          color="#4C6E99"
          text-color="#fff"
          v-if="schoolInfo.lxname=='国际博士'"
        >{{schoolInfo.lxname}}</van-tag>
        <van-tag
          color="#A8BA9A"
          text-color="#fff"
          v-if="schoolInfo.lxname=='高级研修'"
        >{{schoolInfo.lxname}}</van-tag>
      </div>
      <div class="sub_title">
        <i>
    ...
    <div class="hot_major" v-show="this.$route.query.lx==2" style="margin-top:30px">
      <h5 class="left"></h5>
      <span class="major left">招生项目</span>
    </div>

    <van-grid :column-num="3" :border="false" v-show="this.$route.query.lx==2">
      <van-grid-item
        v-if="schoolzxx[1]"
        :text="schoolzxx[0].lxtitle"
        @click="cl_1(schoolzxx[0].major0,schoolzxx[0].major3)"
      >
        <img slot="icon" src="../../../assets/icon/MBA.png" />
      </van-grid-item>
      <van-grid-item
        v-if="schoolzxx[1]"
        :text="schoolzxx[1].lxtitle"
        @click="cl_2(schoolzxx[1].major0,schoolzxx[1].major3)"
      >
        <img slot="icon" src="../../../assets/icon/EMBA.png" />
      </van-grid-item>
      <van-grid-item
        v-if="schoolzxx[2]"
        :text="schoolzxx[2].lxtitle"
        @click="cl_3(schoolzxx[2].major0,schoolzxx[2].major3)"
      >
        <img slot="icon" src="../../../assets/icon/MPAcc.png" />
      </van-grid-item>
      <van-grid-item
        v-if="schoolzxx[3]"
        :text="schoolzxx[3].lxtitle"
        @click="cl_4(schoolzxx[3].major0,schoolzxx[3].major3)"
      >
      <img slot="icon" src="../../../assets/icon/MEM.png" />
      </van-grid-item>
      <van-grid-item
        v-if="schoolzxx[4]"
        :text="schoolzxx[4].lxtitle"
        @click="cl_5(schoolzxx[4].major0,schoolzxx[4].major3)"
      >
        <img slot="icon" src="../../../assets/icon/MPA.png" />
      </van-grid-item>
      <van-grid-item
        v-if="schoolzxx[5]"
        :text="schoolzxx[5].lxtitle"
        @click="cl_6(schoolzxx[5].major0,schoolzxx[5].major3)"
      >
        <img slot="icon" src="../../../assets/icon/MF.png" />
      </van-grid-item>
      <van-grid-item
        v-if="schoolzxx[6]"
        :text="schoolzxx[5].lxtitle"
...
export default {
  ...
  methods: {
    ...
    cl_1 (lx, mar) {
      this.$router.push({ path: `/schoolPro/${this.htmlID}`, query: { id: this.htmlID, lx: lx, mar: mar, zlx: 'MBA' } })
    },
    cl_2 (lx, mar) { this.$router.push({ path: `/schoolPro/${this.htmlID}`, query: { id: this.htmlID, lx: lx, mar: mar, zlx: 'EMBA' } }) },
    cl_3 (lx, mar) { this.$router.push({ path: `/schoolPro/${this.htmlID}`, query: { id: this.htmlID, lx: lx, mar: mar, zlx: 'MPAcc' } }) },
    cl_4 (lx, mar) { this.$router.push({ path: `/schoolPro/${this.htmlID}`, query: { id: this.htmlID, lx: lx, mar: mar, zlx: 'MPA' } }) },
    cl_5 (lx, mar) { this.$router.push({ path: `/schoolPro/${this.htmlID}`, query: { id: this.htmlID, lx: lx, mar: mar, zlx: 'MF' } }) },
    cl_6 (lx, mar) { this.$router.push({ path: `/schoolPro/${this.htmlID}`, query: { id: this.htmlID, lx: lx, mar: mar } }) },
    // 入学须知跳转
    ArticleDetails (id, i) {
    ...
    getRuxue () {
      ...
      this.$http({
        method: 'post',
        url: 'https://www.125yan.com/interface.php?s=/Home/Index/xx_rxxz',
        data: dada
      })
        .then((res) => {
          console.log(res)
